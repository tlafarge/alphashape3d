...
%\VignetteDepends{geometry, rgl, alphahull}
...
<<echo=FALSE, cache=TRUE,fig.width=8,fig.height=3,out.width="0.9\\linewidth" >>=
n<-300
m<-0
data<-matrix(0,n,2)
while(m<n){
...
<<rgl=TRUE, dev='png',echo=FALSE, results='hide',message=FALSE, fig.width=12, fig.height=12,units = "cm",resolution=320,out.width="0.8\\linewidth">>=
...
<<echo=FALSE, fig.width=5, fig.height=5,units = "cm">>=
par(mfrow=c(1,1))
plot(0,type="n",xlim=c(0,1),ylim=c(0,1),xlab="",ylab="",axes=F)
plot(delvor(x),col=1:3,xlab="",ylab="",add=T)
@
...
Graphical visualization is an essential part of data analysis and statistical modelling. The function \code{plot} (S3 method for class
...
functionality to the \proglang{R} programming environment. Figure~\ref{fig:ahull4b} shows snapshots of the above call to \code{plot}. 
...
\vspace{-1.5cm}
\end{center} 
\caption{Snapshots from \code{plot} representing the $\alpha$-shape of the point cloud for $\alpha=\Sexpr{alp1}$ (left) and $\alpha=\Sexpr{alp2}$ (right).}
\label{fig:ahull4b}
\end{figure}
...
This shows that the $\alpha$-shape for $\alpha=0.3$ has two
  ...
  component. The function \code{plot}, described in Subsection \ref{sec:graph}, displays the connected components of the $\alpha$-shape in different colours by setting the argument \code{byComponents = TRUE}. The following code produces Figure~\ref{fig:comp}.
