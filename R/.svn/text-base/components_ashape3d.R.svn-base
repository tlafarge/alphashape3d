components_ashape3d<-function(as3d,indexAlpha = 1){
	...
	for (iAlpha in indexAlpha)
	{
		...
		edgeSelectResult=.C("edgeselect", n=as.integer(nbPoints), x=as.double(as3d$x), ed=as.integer(as3d$edge[,1:2]),
				nb=as.integer(dim(as3d$edge)[1]),alpha=as.double(as3d$alpha[iAlpha]),nbfinal=as.integer(0), PACKAGE="alphashape3d")
		...
		if (dim(edges)[1] > 0)
			...
			{
				adjacency[[edges[ij,1]]] = append(adjacency[[edges[ij,1]]],edges[ij,2])
				adjacency[[edges[ij,2]]] = append(adjacency[[edges[ij,2]]],edges[ij,1])
			}
		...
		label=matrix(as.numeric(as.matrix(as.data.frame(table(visited)))),ncol=2)
		if(any(label[,1]==-1))
		...
		if(dim(label)[1]>1)
		{
			...
			for(ii in 1:dim(label)[1])
				visited[visited==label[ii,1]]=-ii
